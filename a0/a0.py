...
def get_users(twitter, screen_names):
    ...
    [6253282, 783214]
    """
    request=robust_request(twitter, 'users/lookup', {'screen_name': screen_names}, 5)
...
def get_friends(twitter, screen_name):
    ...
    """
    request=robust_request(twitter, 'friends/ids', {'screen_name': screen_name}, 5)
    z=[x for x in request]
    return z
...
def print_num_friends(users):
    ...
    Returns:
        Nothing
    """
    commonfriends=defaultdict(list)
    for i in range(len(users)):
...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(i+1,len(users)):
            C=list(set(users[i]['friends']).intersection(users[j]['friends']))
            zz[users[i]['screen_name'],users[j]['screen_name'],len(C)].append(len(C))
    _tuples=[x for x,v in zz.items()]
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    #print("ASAD",C[0],C)
    request=robust_request(twitter, 'users/lookup', {'user_id': C[0]}, 5)
...
def create_graph(users, friend_counts):
    ...
    for k,v in dict(friend_counts).items():
        if v>1:
            #print(k)
            G.add_node(k)
            for i in range(len(users)):
                for x in users[i]['friends']:
                    if(x==k):
                        G.add_edge(users[i]['screen_name'], k)
    return G
...
def draw_network(graph, users, filename):
    ...
    CC=defaultdict(list)
    for i in range(len(users)):
        CC[users[i]['screen_name']].append(users[i]['screen_name'])
    plt.figure(figsize=(10,10))
    nx.draw_networkx(graph,node_size=20,with_labels=True,font_size=10,labels=CC,edge_color='y')
